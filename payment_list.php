		...
		$('input:checkbox').change(function() {
			...
			for(var i=0, n=order_id.length;i<n;i++) {
					if (order_id[i].checked) {
						...
						//13/04/2017 Prathaya Ch. hidden package_id[]
						if (!isHidden) {
							package_id = document.getElementsByName('package_id[]');
							for(var j=0, k=package_id.length;j<k;j++) {
								package_id[j].style.display = "none";
							}
							isHidden = true;
						}
					}
			...
			for(var i=0, n=package_id.length;i<n;i++) {
					if (package_id[i].checked) {
						...
						//13/04/2017 Prathaya Ch. hidden order_id[]
						if (!isHidden) {
							order_id = document.getElementsByName('order_id[]');
							for(var j=0, k=order_id.length;j<k;j++) {
								order_id[j].style.display = "none";
							}
							isHidden = true;
						}
					}
			...
			//13/04/2017 Prathaya Ch. redisplay
